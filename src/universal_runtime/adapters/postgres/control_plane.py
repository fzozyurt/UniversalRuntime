...
class PostgresControlPlaneCatalog(ApplicationDeploymentCatalog, ExecutionPlanResolver):
    def __init__(
        self,
            ...
            async with session.begin():
                ...
                await session.execute(
                    text(
                        ...
                        "(id, workspace_id, project_id, name, environment, active_revision_id) "
                ...
                await session.execute(
                    text(
                        ...
                        "VALUES (:id, :application_id, :revision_id, :environment, 'ready') "
                        "ON CONFLICT (id) DO UPDATE SET application_id = EXCLUDED.application_id, "
                        "revision_id = EXCLUDED.revision_id, environment = EXCLUDED.environment, "
                ...
                for graph in registration.graphs:
                    graph_revision_id = f"{registration.revision_id}:{graph.graph_id}"
                    ...
                    await session.execute(
                        text(
                            ...
                            "descriptor_hash) VALUES (:id, :application_id, :revision_id, :graph_id, "
                            ":entrypoint, CAST(:descriptor AS jsonb), :descriptor_hash) "
                    ...
                    existing_graph = (
                        await session.execute(
                            text(
                                "SELECT descriptor_hash FROM rt_core.graph_revisions WHERE id = :id"
                            ),
                            {"id": graph_revision_id},
                        )
                    ).scalar_one()
                    ...
                    metadata = {
                        ...
                        "runtime.application_id": str(registration.application_id),
                        "runtime.revision_id": str(registration.revision_id),
                        "runtime.deployment_id": str(registration.deployment_id),
                        "name": graph.graph_id,
                    }
                    await session.execute(
                        text(
                            "INSERT INTO rt_core.assistant_versions "
                            "(id, assistant_id, version, config, context, metadata) "
                            "VALUES (:id, :assistant_id, 1, '{}'::jsonb, '{}'::jsonb, "
                            "CAST(:metadata AS jsonb)) ON CONFLICT (assistant_id, version) DO NOTHING"
                        ),
                        {
                            "id": f"{assistant_id}:1",
